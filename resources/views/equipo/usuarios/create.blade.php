...
<div class="card">
  ...
  <form method="POST" action="{{ route('equipo.usuarios.store') }}">
    @csrf
    ...
    <h3 style="margin:0 0 10px;">Perfil (opcional)</h3>

    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
      <div><label>País</label><input class="input" name="country" value="{{ old('country') }}"></div>
      <div><label>Ciudad</label><input class="input" name="city" value="{{ old('city') }}"></div>
      <div><label>Código postal</label><input class="input" name="postal_code" value="{{ old('postal_code') }}"></div>
      <div><label>Tipo doc</label><input class="input" name="document_type" value="{{ old('document_type') }}"></div>
      <div><label>Número doc</label><input class="input" name="document_number" value="{{ old('document_number') }}"></div>
      <div><label>Teléfono</label><input class="input" name="phone" value="{{ old('phone') }}"></div>
      <div style="grid-column:1/3;"><label>Bio</label><input class="input" name="bio" value="{{ old('bio') }}"></div>
    </div>
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // restaurar old(company_id)
    const oldId = document.getElementById('company_id').value;
    if (oldId) {
      const btn = document.querySelector('.company-row[data-id="' + oldId + '"]');
      if (btn) {
        document.getElementById('company_label').value =
